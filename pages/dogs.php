...
<html lang="en">
  <head>
    ...
    <title> Meet the Competitors - The Poppleton Dog Show </title>
...
<?php
    ...
    $query = $db->prepare("SELECT dogs.name, owner_id as id, breeds.name as breed FROM dogs, breeds WHERE dogs.breed_id = breeds.id ORDER BY dogs.name ASC;");
    $query->execute();
    ...
    while($row = $result->fetch_assoc()){
        ...
        if($row["name"][0] != $currLetter){ // Detect when we are in a different section alphabetically

            $currLetter = $row["name"][0]; 
            echo("<h2> " .$currLetter. " </h2>"); // Output letter headings
        }

        // Output dog and breed
        echo("<a href = 'profile.php?owner=" .$row["id"]." '> " .$row["name"]. "</a> (" .$row["breed"]. ") <br>");
